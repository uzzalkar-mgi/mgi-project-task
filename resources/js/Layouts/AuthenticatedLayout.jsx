...
function ThemeToggle() {
    ...
    const toggle = () => {
        ...
        try { localStorage.theme = next ? 'dark' : 'light'; } catch (e) { /* ignore */ }
    };
    return (
        <button onClick={toggle} title={dark ? 'Light mode' : 'Dark mode'} className="rounded p-2 hover:bg-white/10">
            {dark ? (
                ...
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.8A9 9 0 1111.2 3a7 7 0 009.8 9.8z" /></svg>
            )}
        </button>
    );
}
