...
export default function Form({ user, roles, departments, designations, assigned }) {
    ...
    return (
        <AuthenticatedLayout
            header={
                <PageHeader
                    title={editing ? `Edit User: ${user.name}` : 'New User'}
                    subtitle="Create an account and assign one or more roles."
                    actions={
                        <Link href={editing ? route('users.show', user.uuid) : route('users.index')} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">
